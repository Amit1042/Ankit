    ...
    $.ajax({
        ...
        success: function (data) 
        {
            ...
            {	
            	...
            	const distinctItems = [...new Map(AnalyticalUsageData.map(item => [item["AuthorId"], item])).values()];
				...
				const uniquewebuser = [...new Map(FilteredValueweb.map(item => [item["AuthorId"], item])).values()];				
				...
				const uniquemobuser = [...new Map(FilteredValuemob.map(item => [item["AuthorId"], item])).values()];    			
...
function ToGetCountbyDate(DateArray)
{
	...
	for(var i=0; i<DateArray.length; i++)
	{
		var ExtResults= AnalyticalUsageData.filter(function (data) {
       	...
       	{       	
       		const distinctItems = [...new Map(ExtResults.map(item => [item["AuthorId"], item])).values()];
...
function GetAllVisitedUserss()
{
    ...
    const ActiveuniqueUsers = [...new Map(AnalyticalUsageData.map(item => [item["AuthorId"], item])).values()];
    		ActiveuniqueUsers .sort(function(a,b){return a.Author.Title< b.Author.Title? -1 : 1});
    ...
    if(ActiveuniqueUsers.length>0)
    {
    	...
    	{
    		var FilteredRec = $.grep(AnalyticalUsageData, function(v) {
    			return v.AuthorId == ActiveuniqueUsers[i].AuthorId;
			});
			
			var TotalVisit = FilteredRec.length;
			var LastRecord = FilteredRec[(FilteredRec.length-1)].Created;
			var UserName = FilteredRec[(FilteredRec.length-1)].Author.Title;
			var UserEmail = FilteredRec[(FilteredRec.length-1)].Author.EMail;
			...
			var UserInfo = $.grep(AnalyticalActiveUsers, function(v) {
    			return v.LogonName.ID == ActiveuniqueUsers[i].AuthorId;
			});
			var UserDesignation ='';
			var UserDept = '';
...
function GetAllWebsiteVisitedUserss()
{
	...
	const ActiveuniqueUsers= [...new Map(FilteredValueweb.map(item => [item["AuthorId"], item])).values()];
    	  ActiveuniqueUsers.sort(function(a,b){return a.Author.Title< b.Author.Title? -1 : 1});
	...
	if(ActiveuniqueUsers.length>0)
	{
		for(var i=0; i<ActiveuniqueUsers.length; i++)
		{
			...
			var UserName = FilteredRec[(FilteredRec.length-1)].Author.Title;
			var UserEmail = FilteredRec[(FilteredRec.length-1)].Author.EMail;
			var UserImage = '../SiteAssets/WebAnalytics/assets/images/user_pic.jpg';
...
function GetAllMobileVisitedUserss()
{
	...
	const ActiveuniqueUsers= [...new Map(FilteredValuemob.map(item => [item["AuthorId"], item])).values()];
    	  ActiveuniqueUsers.sort(function(a,b){return a.Author.Title< b.Author.Title? -1 : 1});
	...
	if(ActiveuniqueUsers.length>0)
	{
		for(var i=0; i<ActiveuniqueUsers.length; i++)
		{
			...
			{
				UserImage = _spPageContextInfo.webAbsoluteUrl + '/_layouts/15/userphoto.aspx?accountname=' + UserEmail//"../SiteAssets/EmployeeSynchronous/EmployeeDirectory/user_pic.jpg";
			}
			var TotalVisit = FilteredRec.length;
			var LastRecord = FilteredRec[(FilteredRec.length-1)].Created;
...
function BindNonActiveUsers(Action)
{
	...
	if(Action == '')
	{
		...
		var result = AnalyticalActiveUsers.filter(function (o1) {
    		return !distinctItems.some(function (o2) {
    	    	return o1.LogonNameId== o2.AuthorId; // return the ones with equal id
   			});
		});
		if(result.length>0)
		{	
	...
	else if(Action == 'Website')
	{
		...
		const distinctItems = [...new Map(FilteredValueweb.map(item => [item["AuthorId"], item])).values()];
		var result = AnalyticalActiveUsers.filter(function (o1) {
    		return !distinctItems.some(function (o2) {
    	    	return o1.LogonNameId== o2.AuthorId; // return the ones with equal id
   			});
...
function GetVisitedPages()
{
    ...
    {	
    	...
    	var TotalVisit = FilteredRec.length;
    	const DistinctUsers = [...new Map(FilteredRec.map(item => [item["AuthorId"], item])).values()];
...
function getpopDetails(DataName, App,Condition)
{
		...
		if(DataResult.length>0)
		{
			for(i=0;i<DataResult.length;i++)
    		{
    		    ...
    		    //Deatils += '<tr class="text-left"><td class="text-left"><div class="analytical-user-image-box"><img src="'+DataResult[i].UserImage+'"><div class="analytical-user-image-text"><p class="mb0">'+DataResult[i].UserID.Title+'</p><p class="mb0">'+DataResult[i].Designation+' | '+DataResult[i].DepartmentId.Title+'</p></div></div></td><td class="text-left">'+moment(DataResult[i].Modified).format('DD-MMM-YYYY HH:mm')+'</td><td>'+DataResult[i].WebpartName+'</td><td>'+DataResult[i].Title+'</td><td>'+DataResult[i].Application+'</td></tr>';
    		      Deatils += '<tr class="text-left activities-tr"><td class="text-left"><div class="analytical-user-image-box"><img src="'+DataResult[i].UserImage+'"><div class="analytical-user-image-text"><p class="mb0">'+DataResult[i].UserID.Title+'</p><p class="mb0">'+DataResult[i].Designation+' | '+DataResult[i].DepartmentId.Title+'</p><p class="mb0 Emailfont">'+DataResult[i].Author.EMail+'</p></div></div></td><td class="text-left">'+moment(DataResult[i].Modified).format('DD-MMM-YYYY HH:mm')+'</td><td>'+DataResult[i].WebpartName+'</td><td>'+DataResult[i].Title+'</td><td>'+DataResult[i].Application+'</td></tr>';
    		}
    	}
    ...
    else if(App == "OverAllUserList")
    {
    	...
    	const ActiveUserList = [...new Map(DataResult.map(item => [item["AuthorId"], item])).values()];
    	var tblBodyData = '';
    	if(ActiveUserList.length>0)
    	{
    		for(var i=0; i<ActiveUserList.length; i++)
    		{
    			...
    			tblBodyData += '<tr class="text-left"><td class="text-left js-sort-number"><div class="analytical-user-image-box"><img src="'+UserImage+'"> <div class="analytical-user-image-text"><p class="mb0">'+ActiveUserList[i].Author.Title+', </p><p class="mb0">'+UserDesignation+' | '+UserDept+'</p><p class="mb0 Emailfont">'+ActiveUserList[i].Author.EMail+' </p></div></div></td></tr>'					   		
    		}    	
    	}
    	$('#tbdyNonActiveuser').append(tblBodyData);    
    }
